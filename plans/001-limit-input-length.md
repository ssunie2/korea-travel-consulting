...
`/api/plan` 은 손님이 보낸 값을 그대로 GPT-6 Astra 프롬프트에 끼워 넣는다. 그런데 **문자열 길이 상한이 없다.**
`destinations` 와 `styles` 는 배열 개수만 제한하고(각각 10개·6개) 각 원소의 길이는 무제한이다.

실제로 확인했다 — 20만 자짜리 도시 이름이 검사를 그대로 통과한다.

OpenAI API는 사용량에 따라 비용이 생기므로, **이 구멍을 막지 않으면 긴 입력이 그대로 실제 요금이 된다.**
사용 한도에 닿으면 **손님 전원의 초안 생성도 멈춘다.**
...
- **OpenAI API 키를 운영 환경에 넣기 전에 이 계획이 먼저 들어가 있어야 한다.** 순서가 바뀌면 그 사이에 요금이 샌다
